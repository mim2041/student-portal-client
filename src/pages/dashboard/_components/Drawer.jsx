  ...
  const handleLogout = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        localStorage.removeItem("accessToken_JU");
          ...
          <article className="relative w-full pb-10 flex flex-col h-full">
            ...
            <div className=" mt-8 flex flex-col">
              {routes.map((nav, index) => (
                <NavLink
                  key={nav.path}
                  className={` flex items-center w-full my-1 font-poppins  cursor-pointer text-start py-3 rounded-e-full font-light ${
                    active === nav.title
          ...
          <article className="relative w-full pb-10 flex flex-col space-y-6 overflow-y-scroll h-full">
            ...
            <div className=" mt-8 me-4 flex flex-col">
              {routes.map((nav, index) => (
                <NavLink
                  key={nav.path}
                  className={`flex items-center justify-start ps-4 w-full my-1 font-poppins font-medium cursor-pointer text-start py-3 rounded-e-full text-[15px] ${
                    active === nav.title
                      ? " text-white bg-first/80"
